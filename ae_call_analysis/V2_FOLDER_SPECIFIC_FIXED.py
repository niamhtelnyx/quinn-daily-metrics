#!/usr/bin/env python3
"""
V2 FINAL Call Intelligence - FOLDER-SPECIFIC SEARCH (FIXED)
...
📁 FOLDER-SPECIFIC Google Drive parsing ✅ (FIXED)
✅ Smart deduplication
"""
...
def extract_attendees_from_content(content, title=""):
    ...
    try:
        ...
        # PRIORITY 4: Content-based company extraction
        if prospect_name == 'Unknown Prospect':
            content_patterns = [
                r'([A-Z][a-zA-Z\s&]{2,15})\s+(?:team|discussed|explained|confirmed)',
                r'([A-Z][a-zA-Z\s&]{2,15})\s+<>\s+Telnyx',
                r'Meeting\s+with\s+([A-Z][a-zA-Z\s&]{2,15})'
            ]
            
            for pattern in content_patterns:
                matches = re.findall(pattern, content)
                for match in matches:
                    if 'telnyx' not in match.lower() and len(match.strip()) > 2:
                        prospect_name = match.strip().title()
                        break
                if prospect_name != 'Unknown Prospect':
                    break
