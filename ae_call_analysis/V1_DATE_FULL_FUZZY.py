#!/usr/bin/env python3
"""
V1 Enhanced Call Intelligence - Date Hierarchy with FUZZY MATCHING
Same as V1_DATE_FULL.py but with fuzzy Salesforce event matching

FIXES: Google Drive folder names don't exactly match Salesforce event subjects
SOLUTION: Try multiple search patterns and fuzzy matching
"""
...
def find_salesforce_event_fuzzy_match(access_token, event_name):
    """NEW: Find Salesforce event with fuzzy matching"""
    try:
        domain = os.getenv('SF_DOMAIN', 'telnyx')
        ...
        # Strategy 2: Try fuzzy matching with key words
        # Extract key words from event name (remove common connector words)
        key_words = []
        words = re.findall(r'\b\w+\b', event_name.lower())
        skip_words = ['and', 'the', 'with', 'for', 'to', 'of', 'in', 'at', 'on', 'by', '-', '--', '&']
        ...
        if len(key_words) >= 2:
            # Build a SOQL query that searches for events containing multiple key words
            like_conditions = []
            for word in key_words[:3]:  # Use first 3 key words
                escaped_word = word.replace("'", "\\'")
                like_conditions.append(f"Subject LIKE '%{escaped_word}%'")
            ...
            log_message(f"🔍 FUZZY search for keywords: {key_words[:3]}", False)
            ...
            if response.status_code == 200:
                ...
                if results['records']:
                    ...
                    log_message(f"🎯 FUZZY match found: {best_match['Subject']}", False)
                    return best_match
...
def init_database():
    ...
    conn = sqlite3.connect('v1_date_fuzzy.db')
    cursor = conn.cursor()
...
def mark_as_processed(call_id, dedup_key, event_name, prospect_name, company_name, slack_posted, ai_analysis, salesforce_event_id=None, content_length=0, match_type="exact"):
    ...
    cursor.execute('''
        ...
        (call_id, dedup_key, event_name, prospect_name, company_name, processed_at, source, slack_posted, ai_analysis, salesforce_event_id, content_length, match_type)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (call_id, dedup_key, event_name, prospect_name, company_name, 
          datetime.now().isoformat(), 'google_drive_date_hierarchy_fuzzy', slack_posted, json.dumps(ai_analysis), salesforce_event_id, content_length, match_type))
    
    conn.commit()
    conn.close()

def process_date_hierarchy_fuzzy(main_folder_id="1i0Vh_dTjkULE4VwVDSYlFrgnLTxXJoUY", days_back=1, max_meetings_per_day=15):
    """FUZZY processing pipeline with better Salesforce matching"""
    
    log_message("🚀 V1 DATE FUZZY - Complete Pipeline with Fuzzy Salesforce Matching")
    ...
    for date_folder in date_folders:
        ...
        for meeting in meeting_folders:
            ...
            for gemini_file in gemini_files:
                ...
                # NEW: Use fuzzy matching
                event_record = find_salesforce_event_fuzzy_match(access_token, event_name)
                
                if not event_record:
                    log_message(f"      🔍 Event: ❌ No fuzzy matches found", False)
                    continue
                
                # Determine match type for tracking
                match_type = "exact" if f"Meeting Booked: {event_name}" == event_record['Subject'] else "fuzzy"
                if match_type == "exact":
                    total_exact_matches += 1
                else:
                    total_fuzzy_matches += 1
                
                log_message(f"      🔍 Event: ✅ Found Salesforce event ({match_type})", False)
    ...
    log_message(f"   🎯 Exact matches: {total_exact_matches}")
    log_message(f"   🔍 Fuzzy matches: {total_fuzzy_matches}")
    log_message(f"   💾 Database: v1_date_fuzzy.db")

if __name__ == "__main__":
    process_date_hierarchy_fuzzy()