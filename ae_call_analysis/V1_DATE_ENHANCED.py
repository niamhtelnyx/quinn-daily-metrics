#!/usr/bin/env python3
"""
V1 Enhanced Call Intelligence - Date Hierarchy System
Processes Google Drive meetings organized by date folders with full V1 functionality

NEW STRUCTURE SUPPORT:
Meeting Notes/
├── 2026-03-05/                     (Date folders)
│   ├── Meeting Name 1/              (Individual meeting folders)  
│   │   ├── Gemini Notes            ("Copy of [Name] - Date/Time - Notes by Gemini")
│   │   └── Recording (optional)    ("[Name] - Date/Time - Recording")
│   └── Meeting Name 2/
└── 2026-03-04/

FULL V1 FEATURES:
✅ Date hierarchy support
✅ Event name extraction from meeting folder names
✅ Salesforce event lookup with exact subject matching 
✅ AI analysis with OpenAI
✅ Slack posting with threading
...
def get_salesforce_token():
    ...
    auth_url = f"https://{os.getenv('SALESFORCE_DOMAIN')}/services/oauth2/token"
...
def find_salesforce_event_by_exact_subject(access_token, event_name):
    ...
    instance_url = f"https://{os.getenv('SALESFORCE_DOMAIN')}"
...
def init_database():
    ...
    conn = sqlite3.connect('v1_date_enhanced.db')
    cursor = conn.cursor()
...
def mark_as_processed(call_id, dedup_key, event_name, prospect_name, company_name, slack_posted, ai_analysis):
    ...
    cursor.execute('''
        ...
        (call_id, dedup_key, event_name, prospect_name, company_name, processed_at, source, slack_posted, ai_analysis)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (call_id, dedup_key, event_name, prospect_name, company_name, 
          datetime.now().isoformat(), 'google_drive_date_hierarchy', slack_posted, json.dumps(ai_analysis)))
    
    conn.commit()
    conn.close()

def process_date_hierarchy_meetings(main_folder_id="1i0Vh_dTjkULE4VwVDSYlFrgnLTxXJoUY", days_back=1, max_meetings_per_day=15):
    """Main processing function for date hierarchy structure with full V1 functionality"""
    
    log_message("🚀 V1 DATE ENHANCED Call Intelligence - Full Processing Pipeline")
