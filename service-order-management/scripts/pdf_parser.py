...
def analyze_ramp_schedule_variation(schedule: List[Dict], raw_commitment_text: str) -> Dict[str, Any]:
    """
    Analyze a ramped commitment schedule to detect template vs reality mismatches.
    
    Returns analysis indicating whether this is truly ramped (amounts vary) 
    or static (flat amounts using ramped template).
    """
    if not schedule:
        return {"is_truly_ramped": False, "reason": "No schedule data"}
    
    # Extract all amounts from the schedule
    amounts = [commit["amount"] for commit in schedule]
    unique_amounts = set(amounts)
    
    # Analysis results
    is_truly_ramped = len(unique_amounts) > 1
    total_periods = len(schedule)
    min_amount = min(amounts) if amounts else 0
    max_amount = max(amounts) if amounts else 0
    
    # Extract the actual table from the contract for proof
    table_lines = []
    for line in raw_commitment_text.split('\n'):
        line = line.strip()
        if ('Commit Start Date' in line or 
            'April 1, 2026' in line or
            'calendar month thereafter' in line or
            ('$' in line and ('2026' in line or 'thereafter' in line))):
            table_lines.append(line)
    
    # Build analysis result
    analysis = {
        "is_truly_ramped": is_truly_ramped,
        "total_periods": total_periods,
        "unique_amounts": len(unique_amounts),
        "amount_range": {
            "min": min_amount,
            "max": max_amount,
            "variation": max_amount - min_amount
        },
        "evidence_table": table_lines,
        "raw_amounts": amounts,
        "conclusion": "",
        "recommendation": ""
    }
    ...
    if is_truly_ramped:
        analysis["conclusion"] = f"TRUE RAMP: Found {len(unique_amounts)} different amounts across {total_periods} periods"
        analysis["recommendation"] = "Process as ramped commitment with Service_Order_Details__c records"
    else:
        analysis["conclusion"] = f"FLAT RAMP: Same amount (${amounts[0]:,.2f}) across all periods"
        analysis["recommendation"] = "Process as static commitment despite 'ramped' template label"
...
def extract_commitment_data(text: str) -> Dict[str, Any]:
    ...
    if ramped_section:
        ...
        # ENHANCEMENT: Validate ramp schedule for actual amount variation
        ramp_analysis = analyze_ramp_schedule_variation(commitment_schedule, ramped_text)
...
def parse_service_order(text: str) -> Dict[str, Any]:
    ...
    if commitment_data["type"] == "ramped":
        ...
        # ENHANCEMENT: Add ramp analysis to metadata
        if "ramp_analysis" in commitment_data:
            result["_metadata"]["ramp_analysis"] = commitment_data["ramp_analysis"]
